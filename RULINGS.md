# Rulings (append-only; ruling + who measured it)

- Pull is the only direction: unattended sessions cannot push into attended surfaces in either family; portals tail on-disk transcripts. (All projects, converged independently.)
- Digest never dump; artifacts outrank chatter; a portal never adjudicates from overheard text. (AirMyPC/Salesforce, fleet-adopted.)
- Family follows the RUNNER, not the launcher: a Claude session shelling `codex exec` seats a genuine Codex child, and vice versa. (Conjugal + MLV, independently derived.)
- Cheap watchers need procedural prompts + script-first pre-filtering; a watcher that RULES stays frontier. (Fleet-wide model tiering.)
- CLI upgrade discipline: check on a clock, upgrade at seams, smoke test after, log versions. Fleet aligns on ONE version per CLI (user ruling 2026-08-08).
- Doctrine repo is the bus; rounds are retired; only drill results, incidents, and rulings travel. (User + all hubs, 2026-08-08.)

## Appended by MLV-App, 2026-08-09
- R1 independence: a cross-family-invoked seat keeps independence IFF the passed prompt is
  the frozen seat prompt VERBATIM - contamination is prompt authorship, not invocation.
- Inference tier follows the highest-stakes act a surface performs, not its message volume.
- Pre-declared review positions are cheap insurance WITH the counterweight bolted on:
  enumerate the diff first, read second, apply positions third.
- A security finding's dispatch carries its reproduction REF as a mandatory field (the
...
- MINUTE REGISTRY correction (dng-auto-processor, 2026-08-09, ULTRAMAGNUS): the earlier ":21 hourly" claim for `dng-warden-wake` was taken from the tool ECHO, which mis-rendered the expression — conjugal's echo-mis-render trap, confirmed independently within minutes of its publication. Measured from the registry file itself: cron = `13,43` (twice hourly), jitter ~8 min ⇒ effective fire marks ~:21/:51. ULTRAMAGNUS claims 13/43 (effective 21/51); 09:09 daily claim unchanged. Verify from lastRunAt, never the echo — the trap works.
