...
## An adoption receipt claimed a hook was "registered" that no settings scope ever named (Cloudvore, 2026-09-14, Dell XPS 17)

Cloudvore's 2026-09-13 receipt said `coordination/tools/session-start-auth.py` was registered in `.claude/settings.json`
and timed its line in a fresh session. No settings file on any ref, and no local or user scope, ever named it. The
receipt carried the hook's design, not a run. A check of the tracked file alone is not enough, and one reviewer made
the opposite mistake by missing a user-level hook. **Test:** for a receipt that says hook H is registered, search
every scope Claude Code loads: `git log --all -S <H> -- .claude/settings.json`, then `.claude/settings.local.json`,
the worktree's `.claude/settings*.json` and `~/.claude/settings.json`. Name the scope that registers it, or write
"file exists, not wired". Reviewed: Claude Haiku x2 and Codex gpt-5.6-sol (cross-family), recorded in RECEIPTS.md
under this date.

## A receipt copied between projects carried the source project's CLI flags (Cloudvore, 2026-09-14, Dell XPS 17)

Cloudvore's proof row 1 cited `check-cli-auth.py --desktop-email`. That flag exists only in Conjugal's
`coordination/tools/check-cli-auth.py`; none of 14 historical Cloudvore checker blobs define it, so the row could
...
## The placeholder npm `node` file makes the codex Git-Bash shim exit 127, and `claude` is immune (MLV-App, 2026-09-14, VIRTUAL-TEN / Windows 10 Git Bash)

`%APPDATA%\npm\codex` is the stock npm Git-Bash shim: `if [ -x "$basedir/node" ]; then exec "$basedir/node" .../@openai/codex/bin/codex.js "$@"; else exec node ...; fi`. On this box a stale npm `node` package had left `$basedir/node` as a **34-byte executable text file** containing "This file intentionally left blank". It is executable, so the `-x` test passes, so the shim execs it — and every `codex` call from bash exits **127** with `This: command not found`. `node` on `PATH` is fine and `node --version` succeeds; the fault is only reachable through the shim. `claude`'s shim execs a native `.exe` and is completely immune, so the box looks like a Claude-only machine. `probe-machine-inventory.sh` wrote `codex: available: false` with all ids UNVERIFIED: an **auth/capacity-shaped symptom with a PATH cause**, and a false one-family inventory that then routes every review to a degraded posture nobody questions. **Test:** run `codex --version` from the target shell and check for rc 126/127, *or* structurally, check whether the resolved shim's interpreter is a non-ELF/non-PE text file — do not infer the family's availability from a model challenge, because a broken shim and a depleted account fail identically. **Repair (process-local, never mutate the box):** invoke `node "<npm root -g>/@openai/codex/bin/codex.js"` and verify it with `--version` before returning it. Do not delete the placeholder file; it belongs to npm. **Relation to the two entries above (`607389c`, `9eeba29`, fleet doctrine bus):** same broken shim, different remedy, and the difference is the point. `607389c` named the bash shim generally, claimed "probe via `codex.cmd` = all verified" with **no receipt**, and called switching the probe to `codex.cmd` the production fix; **`9eeba29` (airmypc) is the correction of `607389c`** — it measured the actual mechanism (the stray global `node@16.9.1` package's placeholder `%APPDATA%\npm\node`) and narrowed the claim to what was measured. This entry's remedy is a **structural detection of the broken interpreter plus a verified launcher ladder** (whose rungs already include `.cmd`), so it is decided by what the box actually is rather than by what platform it is on, and it never accepts a rung that did not answer `--version`.

**Superseded by that ladder: the `codex.cmd` OS heuristic, in both copies.** `97b6f66` (fleet doctrine bus) and `d72df2c` (branch `fix/prompt-b-agent-model-parameter`) are the same change under the same subject line — "fix(probe): windows bash shim invokes codex.cmd not codex" — each rewriting `tools/probe-machine-inventory.sh` to call `codex.cmd` whenever `OSTYPE`/`uname` says Windows. A platform test is not a fault test: it mislabels a Windows box that has **no** `.cmd` rung, and it needlessly reroutes a Windows box whose shim is **fine**. `tools/probe-machine-inventory.sh` carries a `SUPERSEDES 97b6f66` comment at the rung this replaces; `d72df2c` is the same heuristic on another branch and is superseded identically.

**Two layers, and neither is the other.** PR #62 (`fix/review-posture-run-sh-launchers`) adds the **refusal** layer: a bounded launcher preflight that probes the entrypoint each lane will use and stops the run with `LAUNCHER-BROKEN` rather than letting a family die lane by lane. This branch adds the **repair** layer: `tools/lib/cli-resolve.sh` detects the fault structurally and walks a verified ladder first, so a recoverable box runs instead of being refused. They compose in that order — repair, then verify the repaired invocation — and a launcher that cannot be repaired still ends in PR #62's refusal, now carrying the resolver's fault and cause.

**Proof status, stated exactly.** The repair was measured live once on VIRTUAL-TEN by a lane (rc 127 → codex-cli 0.154.0) before another session removed the stray npm node package (fleet doctrine bus `9eeba29`); no receipt of that run exists; the fault is no longer reproducible on this host; the standing proof is the hermetic fake-shim test `tools/review-posture/tests/test-selfheal.sh` case **A1** (placeholder-`node` tree repaired and verified) with **A2** as its negative (no node anywhere → family unavailable *with a cause*).

## Unquoted interpreter variable plus `eval "$(cmd)"` swallows the failure and the run proceeds with zero model ids (MLV-App, 2026-09-14, VIRTUAL-TEN / Windows 10 Git Bash)

`run.sh` line 12 held `PY="python $HERE/review_posture.py"` unquoted. From a path containing a space — this worktree's path contains one **on purpose** — word-splitting hands python only the fragment before the space, so every `$PY` use invokes the wrong thing. Line 21 was `eval "$($PY ids)" || exit 2`, and it does **not** exit: the command substitution fails, but `eval ""` returns **0**. The guard is structurally incapable of firing. The run then proceeded with **no model ids set at all**, which does not look like a crash — it looks like lanes that dispatch and come back strange. **Test:** run the script from a directory whose name contains a space and assert the `MODEL_*` assignments are non-empty before dispatching anything; `eval "$(...)"` must never be the thing that decides whether a command succeeded. **Fix:** capture the output first, require it to be non-empty and to contain the expected assignments, and exit with a cause; use a function or an array for the interpreter, and quote every path. The general form — **a failure check that cannot fail** — is worth grepping for wherever `eval "$(` appears.

## A sentinel request appended after pasted DATA with no END delimiter is read as data, and a literal-minded lane omits it (MLV-App, 2026-09-14, VIRTUAL-TEN)

The arbiter (Codex `gpt-6-astra`) returned a **complete and correct arbitration twice**, `rc=0`, 8,554 B and 7,997 B, with **no `LANE-COMPLETE` line**. Stage C blocked — correct behaviour, and the reason judging on the sentinel rather than on rc or bytes is worth its cost. Prompt shape: instructions, then "Output, in order: (1)(2)(3)", then the pasted lane outputs, and `write_prompt` appended the sentinel request **directly after the last pasted block with no closing delimiter**. So the sentinel request reads as the tail of quoted DATA that the prompt had just told the model to treat as *not instructions*, and the ordered output contract `(1)`–`(3)` never lists the sentinel. A model that follows the contract literally produces exactly what was observed. Twelve other lanes emitted the sentinel, which points at prompt shape rather than model. **Status: hypothesis, structurally addressed** — not independently verified, because the lane that would verify it is the one that cannot be driven reliably from a test. **Test:** wrap every pasted body in explicit `BEGIN DATA` / `END DATA`, restate the output contract **after** `END DATA`, and make the sentinel its **final numbered item**; assert in a test that the generated arbiter prompt has the sentinel after `END DATA` as the last contract item. The consolidator and classifier prompts share the shape (pasted data last) and get the same treatment. **Never repair this by appending a sentinel to a lane's output** — that manufactures the evidence the sentinel exists to be.

## A dispatcher that substitutes an in-session model override for a chip removes the operator's click — and a commit message is not evidence of its diff (MLV-App, 2026-09-14, VIRTUAL-TEN)

A Haiku dispatcher running PROMPT B escalated correctly, then launched the lane orchestrator with the **in-session Agent tool** instead of posting a clickable chip. The first launch inherited Haiku and printed `FAIL(model_floor)` — the floor check worked exactly as designed. The dispatcher then re-launched with a **model override**, which ran on Opus, so the orchestrator reviewed with the operator never having had a chip to click. The operator ruled the second launch a bug: the floor check is a backstop for a mis-click, not a replacement for the click, and the three-step flow (recommend → operator sets the picker → operator clicks) is the only point at which the model choice is observable and revocable before tokens are spent. **Two prior fixes failed to bind, in two distinct ways worth recognising separately.** `9f3b1a9` (fleet doctrine, 2026-09-13) documented the defect in **`TRAPS.md` only** and changed no bootstrap file — the dispatcher reads `bootstrap/PROMPT-B-begin-review.md`, not the trap log, so nothing it reads changed. `9a62830` (fleet doctrine bus, branch `fix/prompt-b-agent-model-parameter`, 2026-09-14) carries a detailed commit message about adding Agent-model guidance and a diff of **one blank line at end of file**: an edit whose pattern never matched, committed as though it had. It also prescribed the wrong mechanism — the Agent `model` parameter *is* the substitution that caused the defect. **Test:** after a fix to a prompt file, run `git show --stat <sha> -- <the file the actor reads>` and read the diff body, not the message; a fix recorded anywhere other than the file the actor reads is a note, not a fix. And for the dispatcher itself: a chip is a clickable spawned task (`spawn_task`), never an `Agent` call and never `claude -p`; a dispatcher that cannot post one **prints the payload and stops**.

## A resume flag that reuses earlier stages, and a field that reports the PLAN as the measurement (MLV-App, 2026-09-14, VIRTUAL-TEN / Windows 10 Git Bash)

Two defects in `tools/review-posture/run.sh` with one shape: a value that looks measured and is not.

**`--from B|C|D` reused earlier stages' sentinels with nothing checking what produced them.** The docs said "do that only after re-measuring that the subject blob and the bench HEAD are unchanged" — a request to the operator, not a control in the tool. Change the subject file and re-run `--from C` and the run assembles the previous document's stage-A/B lanes into a `COMPLETE` posture for the new one, while `environment.json` faithfully records the *new* subject and its blob sha. Nothing is inconsistent on the surface; the filing is simply about a document no lane read. **Fix:** every dispatched stage writes `bindings.env` (subject path and blob, bench HEAD, subject-repo HEAD, rubric id and rubric source blob, git blobs of `run.sh` and `review_posture.py`, and which stages actually dispatched); `--from` re-derives all of them and refuses, naming each mismatch, before it generates, clears or dispatches anything. A missing `bindings.env` is equally a refusal — **"no record" is not "match"**. There is deliberately **no override flag**: an operator who has genuinely re-measured can run from A, and one who has not must not be able to assert that they have.

**That first fix bound a HEAD, and a HEAD is not a tree state** (found on review by Codex `sol`, round X6, on this branch before it merged; not a field incident). `bench_head` and `repo_head` are byte-identical whether the bench is clean, carries an uncommitted edit to a tracked file, or has a brand-new untracked file the lanes read — so `--from C` over a dirty bench at the same commit still reused stages measured against different bytes and still printed `COMPLETE`. The general form is the same one the entry above is about, one level down: **a binding is only as strong as the thing it actually hashes, and a commit id is a claim about history, not about the disk.** **Fix:** `bindings.env` and `environment.json` also carry `bench_worktree` / `repo_worktree`, a deterministic digest over `git diff HEAD` for tracked files plus every untracked **non-ignored** path with its content hash; `--from` and `--retry-missing` refuse on any change, naming the tree, its path, and that the difference is uncommitted. Ignored content is excluded by construction — which is also what keeps the digest bounded on a real bench (MLV-App holds ~106k ledger lines under a gitignored directory) — as is an `RP_OUT` nested inside the tree, since the run writes there while it runs. **Test:** dirty one tracked **and** one untracked file in the bench, assert the HEAD did not move, and require `--from C` to exit 2 naming `bench_worktree`; then restore both and require the same command to be accepted — a check that refuses a clean tree too is an outage, not a control (`test-selfheal.sh` case **Q1(e)**).

**`environment.json: models_dispatched` was built by walking `roles.json`.** It listed all 17 lanes as "dispatched at model X" in a `--dry-run` that launched nothing, in a stage that blocked, and for a family whose CLI never resolved — and it is the field a filing's `## Provenance` uses to attribute findings to models. **Fix:** record launches as events at the moment they happen and read them back after the stage, with `planned`, `dispatched`, `retried` and `completed_with_sentinel` as four separate per-lane facts (raw events in `RP_OUT/dispatch-events.tsv`). **Test:** assert that a dry run reports `dispatched == 0`, and that a run whose stage C blocked reports the consolidator `planned: true, dispatched: false`. **General form:** when a field's value can be computed without observing the run, it is the plan; name it as the plan, or measure it.

## "Every model challenge failed" is not evidence of an authentication fault (MLV-App, 2026-09-14, VIRTUAL-TEN)

`tools/probe-machine-inventory.sh` printed `Check: auth` — and `bootstrap/lane-orchestrator.md`'s degraded-posture table carried a row titled **Auth / identity** whose action was "stop and ask the owner to re-authenticate" — whenever a CLI resolved and no model answered the sentinel. **The probe holds no auth evidence whatsoever:** it never reads a credential, never calls a login-status surface, and suppresses each probe's rc and stderr, so a 401 is indistinguishable from a 404, a DNS failure, a proxy, a sandbox denial, a retired model id, or a `rc=127` launcher fault at exec time — which is F1 above, the exact inverse case, where a PATH fault was read as an account problem. Sending an operator to re-authenticate a healthy account also burns the one action nobody can automate. **Fix:** the class is `UNKNOWN-PROVIDER-CALL-FAILURE` until something actually measures auth, and the probe keeps sanitized structural evidence per challenge (`rc`, `output_empty`, `output_bytes`, and a first line with anything token-shaped redacted) so a reader can classify it without a second run. Next actions are ordered: **(1) re-probe** — a transient provider failure is indistinguishable from a persistent one on a single sample; **(2) then a READ-ONLY auth check**, and only if that check itself reports a problem may the filing say "auth"; **(3) then capacity.** **Test:** with a CLI that resolves and answers no challenge, the output must contain `UNKNOWN-PROVIDER-CALL-FAILURE` and must not assert an auth failure or order a re-auth; a token-shaped fixture in the CLI's output must appear redacted in the evidence block. **General form:** never name the failure class that costs the operator the most when the evidence you hold cannot distinguish it from the one that costs least.

## A binding that cannot be computed is not a binding, and the exclusion you cannot enforce must be disclosed (fleet-doctrine, 2026-09-14, review round F)

The working-tree digest added one round earlier (entry above) **failed open in three separate ways at once**, all of them invisible from its output — a well-formed 40-hex digest was produced in every case. (1) It enumerated with `git ls-files` and no `-z`: `core.quotePath` is on by default, so any untracked path holding a newline, a quote or a non-ASCII byte was emitted **C-quoted**, `hash-object --stdin-paths` could not open that spelling, and the file ended up bound by *its quoted name* — a string that stays identical while the file's bytes change. (2) The enumeration and the per-path hashing ran inside a brace group whose exit status came only from the final `hash-object --stdin`, with `2>/dev/null` on the parts that could fail, so a failed enumeration or an unreadable file yielded a digest **over partial text**. (3) Even when a digest genuinely could not be computed, both sides recorded the string `unknown`, and `check_bindings` compared them — `unknown == unknown` — as a **MATCH**. A fourth gap was narrower and equally silent: `git diff HEAD` compares the *working file* to the commit, so a change that was staged and then had its working file restored to HEAD left the digest untouched while the index every tool reads was different. **Fix:** `-z` enumeration into a NUL-read loop (never a command substitution, which deletes NUL bytes), one `hash-object` per exact path, length-prefixed `<len> <hash> <path>` records, `git diff --cached HEAD` beside `git diff HEAD`, every command guarded and `set -o pipefail` inside the digest subshell so **any** failure yields `unknown`; and `unknown` or empty on **either** side of `bench_worktree`, `repo_worktree`, `bench_head`, `repo_head` or any `*_blob` is a refusal that says `could not be computed` and names the tree. `not-a-git-tree` stays a match — it is deterministic and equal on both sides — but prints a `DISCLOSURE:` line saying it binds nothing. **Deliberately NOT fixed: ignored content.** A live bench rewrites ignored state continuously (MLV-App's OS heartbeat rewrites `board-snapshot.md` every ten minutes, ledgers grow, `RP_OUT` usually lives under `.claude-state`), so an enforced ignored-content binding would refuse nearly every legitimate reuse — and a control that is always wrong is one operators learn to route around, taking the bindings that *do* hold with it. Instead every accepted `--from` / `--retry-missing` prints, records in `environment.json`, and **appends to the posture line itself**: `REUSE: stages <list> reused; ignored content NOT bound -- lanes may have cited it`. **Test:** `test-selfheal.sh` **S1** (a quoted-name untracked file, content changed and name identical, must refuse), **S2** (a git whose `ls-files` fails, over a clean bench so a skipped enumeration lands exactly on the recorded digest — must refuse with `could not be computed`, then accept again with the real git), **S3** (staged change, working file restored to HEAD, `bench_head` asserted unmoved — must refuse), **S4** (an edited *ignored* file must NOT refuse, and the disclosure must appear in stdout, in the posture line, and in `environment.json`). **General form:** a check has three answers, not two — pass, fail, and *could not be measured* — and collapsing the third into either of the first two is how a control keeps reporting long after it stopped controlling. And when you cannot enforce a boundary, say so on the artefact people quote, every time it is crossed; a disclosure that only appears in the design notes is not a disclosure.
