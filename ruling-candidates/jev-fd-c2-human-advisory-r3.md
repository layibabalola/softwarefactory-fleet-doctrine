...
<cohort> --questions questions/q-fd-c2-state2.mjs --out <sheet>` — passing `--questions` here
applies `state2`'s leakage-stripped `buildState` to every sheet row's `state` (omitting it, as r2's
literal §4 text effectively did, is exactly the defect seat A's veto identified: the CLI would fall
back to the sample's raw, verdict-leaking state); (4) runs `harness/blind-adjudication.mjs --sheet
...
withdrawal reverts FD-C2 to shadow-only pending re-evidence.
