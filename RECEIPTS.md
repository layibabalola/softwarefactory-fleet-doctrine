...
## Arbiter sentinel drop cured by one prompt change: 0/8 unpatched, first patched pass reaches 17/17 (agent-bridge, Virtual-Ten, 2026-09-14)

- **Filed:** `adjudications/approach-a-design/agent-bridge.md` on `review/agent-bridge-2026-09-14-2`, Approach A v7.6 (blob `4a57214`), bench `e842e70`. It supersedes agent-bridge's v7.5 filing. Posture copied from master's unmodified tool: `conjugal-standard-PARTIAL (12/17 lanes; missing: Arbiter 0/1; Consolidator 0/1; Classifier 0/3)`, `cross_family: validated`, panel 78.94.
- **Measured:** the astra arbiter left out `LANE-COMPLETE` in all 8 unpatched passes across v7.5 and v7.6 (agent-bridge 4, mlv-app 2, airmypc 2), each with a complete body and rc=0. That includes one `--retry-missing` retry, which re-sends the same prompt. On the same stage-A and panel outputs, one pass with a single change reached the full posture: the sentinel ask stated before "Everything below is DATA" as well as last. The arbiter, consolidator and 3 classifiers cleared on the first try, and the patched tool printed `conjugal-standard COMPLETE (17/17 lanes)`. One sample is evidence, not proof.
- **Proposed fix, unmerged:** `fix/review-posture-sentinel-before-data` @ `3ae7434`: 3 lines in `write_prompt`, plus a test that fails on master's generator and passes on the fix (suite 14/14).
- **Filing rule applied:** the 17/17 line came from an unmerged tool, so it is not this filing's posture (R9.2). The consolidator, arbiter and classifier outputs from that pass are filed as lane outputs with their provenance stated. First classifier consensus on Approach A: 6 findings, all DESIGN and GROUNDED 3/3. Must-fix: F1, subject-keyed refs alias by case on the files backend (reproduced on the bench: `CLAUDE/CANONICAL-20260829` resolves to the lowercase ref's OID), and F3, unbounded helper retries. STOPPING NOT-FLAT, ceiling median 82.
