...
$specJson = @'
{
  "schema":"fleet-exhausted-model-failback-controls.v2",
  "candidate":"opus-model-failback-r3",
  ...
  "classifier_rules":{
    ...
    "overage_only_rejected_without_allowed_base_below_100":"HOLD_CLASSIFIER_AMBIGUOUS"
  },
  "cases":[
    ...
    {"id":"overage_only_rejected_without_allowed_base_below_100","mutation":"overage rejected without allowed base-window corroboration","expected":"HOLD_CLASSIFIER_AMBIGUOUS","writes":0,"provider_launches":0},
...
Assert-True ($spec.schema -ceq 'fleet-exhausted-model-failback-controls.v2') 'schema'
Assert-True ($spec.candidate -ceq 'opus-model-failback-r3') 'candidate identity'
...
Assert-True ($spec.cases.Count -eq 26) 'exact control count'
...
foreach ($required in @(
    ...
    'overage_only_rejected_without_allowed_base_below_100',
...
Assert-True ($candidate.StartsWith('# Ruling candidate: exhausted-model failback to Opus R3')) 'R3 title'
