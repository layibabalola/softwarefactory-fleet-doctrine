...
- K12 "`harvest-status.py factory-kernel` showing it `HARVESTED`" | By design, not a bug: `tools/harvest-status.py` counts a filing HARVESTED when `<stem>.dispositions.md` names its blob, and its own test asserts that a dispositions file holding only `filing_blob:` is HARVESTED (`tests/test_harvest_status.py`, `test_dispositions_for_current_blob_harvest_it_and_older_blob_is_stale`). The steward's real dispositions do answer every line (adobe-ingester's answers K1–K12, P lines and U lines). So K12's observable is weaker than the steward's practice, and an empty answer file would satisfy it (seat C of Cloudvore's 2026-09-14 swarm corrected this filing's earlier "defect" framing) | REPLACES: "showing it `HARVESTED`" -> "showing it `HARVESTED`, with one disposition line per filed clause line" | PROOF: commit `filing_blob: <blob>` alone as `<stem>.dispositions.md`; the tool prints HARVESTED
