...
`r26-published-project-disposition-intake.json` folds four independently accepted and published
...
object. The phase-3 checker independently binds the published doctrine base, three forward-only
spec-binding commits, the exact four-project set, each central spec blob, and byte-for-byte equality between
...
the four exact allowlisted GitHub URL/ref pairs, uses a bounded temporary repository with prompts
