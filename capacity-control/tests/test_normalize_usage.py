...
class NormalizeTests(unittest.TestCase):
    def test_anthropic_deduplicates_message_snapshots(self):
        ...
        self.assertEqual(302, result["usage"]["input_tokens"])
    ...
    def test_moonshot_counts_only_usage_record_not_duplicate_step_end(self):
        ...
        self.assertEqual(32, result["usage"]["input_tokens"])
        self.assertEqual(20, result["usage"]["cached_input_tokens"])
        self.assertEqual(1, result["usage"]["turns"])
    ...
    def test_missing_native_fields_remain_unknown_not_invented(self):
    ...
    def test_malformed_structured_stream_fails_closed(self):
        with self.assertRaisesRegex(normalizer.NormalizeError, "not structured JSON"):
            normalizer.normalize("anthropic", ["provider warning, not JSON"], META)
