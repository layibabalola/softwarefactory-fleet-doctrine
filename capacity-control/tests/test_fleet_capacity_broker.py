...
class EvaluationTests(unittest.TestCase):
    ...
    def test_quality_and_identity_are_required(self):
        ...
        wrong_prefix = request()
        wrong_prefix["profile"]["quota_domain"] = "xai:sha256:" + "a" * 64
        with self.assertRaisesRegex(broker.BrokerError, "provider prefix"):
            broker.validate_request(wrong_prefix)

    def test_future_window_start_and_incoherent_timeline_hold(self):
        future = snapshot(started=NOW + dt.timedelta(minutes=1))
        self.assertIn("WINDOW_START_FROM_FUTURE", broker.evaluate(request(), future, policy(), [], NOW).reasons)
        incoherent = snapshot()
        incoherent["windows"][0]["resets_at"] = broker.iso(NOW - dt.timedelta(hours=2))
        self.assertIn("WINDOW_TIMELINE_INVALID", broker.evaluate(request(), incoherent, policy(), [], NOW).reasons)
...
class TransactionTests(unittest.TestCase):
    ...
    def test_cli_serializes_contenders_across_os_processes(self):
        count = 10
        barrier = threading.Barrier(count)
        outcomes: list[tuple[int, str, str]] = []
        errors: list[BaseException] = []
        policy_path = pathlib.Path(self.temp.name) / "policy.json"
        policy_path.write_text(json.dumps(policy()), encoding="utf-8")

        def contender(index: int):
            try:
                request_path = pathlib.Path(self.temp.name) / f"request-{index}.json"
                snapshot_path = pathlib.Path(self.temp.name) / f"snapshot-{index}.json"
                request_path.write_text(json.dumps(request(f"process-{index:04d}")), encoding="utf-8")
        ...
        threads = [threading.Thread(target=contender, args=(index,)) for index in range(count)]
        for thread in threads: thread.start()
        for thread in threads: thread.join(timeout=35)
        self.assertFalse(errors)
        self.assertEqual(count, len(outcomes))
        decisions = [json.loads(stdout) for _, stdout, _ in outcomes]
        self.assertEqual(1, sum(item["status"] == "ADMIT" for item in decisions))
        self.assertEqual(count - 1, sum(item["status"] == "HOLD" for item in decisions))
        self.assertTrue(all(code in {0, 23} for code, _, _ in outcomes))
