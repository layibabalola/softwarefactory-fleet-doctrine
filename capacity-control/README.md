...
This v1 reference is host-scoped. Do not place its SQLite database on a network share. If one
provider account is used on more than one host, either designate one independently administered
broker service as the sole domain owner or keep that account unavailable to automatic work until a
transactional distributed backend is separately reviewed. Deliberately different accounts on
different hosts remain different quota domains and do not need a shared lease.
