...
def json_lines(lines: Iterable[str]) -> Iterable[dict[str, Any]]:
    for line_number, line in enumerate(lines, 1):
        ...
        try:
            ...
            raise NormalizeError(f"line {line_number} is not structured JSON") from exc
        if not isinstance(value, dict):
            raise NormalizeError(f"line {line_number} is not a JSON object")
        yield value
...
def normalize_anthropic(events: Iterable[dict[str, Any]]) -> tuple[dict[str, Any], str | None, str | None]:
    ...
    for event in events:
        ...
        # Normalized input_tokens is total context input; cached_input_tokens is its cached subset.
        usage["input_tokens"] += input_other + cache_create + cache_read
            ...
            if isinstance(info, dict) and isinstance(info.get("total_token_usage"), dict):
                ...
                if isinstance(last, dict):
                    ...
                    usage["peak_context_tokens"] = max(
                        int(usage["peak_context_tokens"]), int(last.get("input_tokens") or 0)
                    )
    if last_total:
...
def normalize_moonshot(events: Iterable[dict[str, Any]]) -> tuple[dict[str, Any], str | None, str | None]:
    ...
    for event in events:
        ...
        usage["input_tokens"] += other + created + cached
...
def normalize(provider: str, lines: Iterable[str], metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in ("ts", "project", "quota_domain"):
        if not isinstance(metadata.get(field), str) or not metadata[field]:
            raise NormalizeError(f"metadata.{field} is required")
    if not isinstance(requested_model, str) or not requested_model:
        raise NormalizeError("metadata.requested_model is required")
    if usage["turns"] == 0:
        for field in ("input_tokens", "cached_input_tokens", "output_tokens", "reasoning_tokens", "peak_context_tokens"):
            usage[field] = None
    return {
        "schema": SCHEMA,
        "ts": metadata["ts"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except (OSError, KeyError, ValueError, TypeError, json.JSONDecodeError, NormalizeError) as exc:
