...
def validate_request(request: dict[str, Any]) -> None:
    ...
    if not isinstance(request["request_id"], str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{7,127}", request["request_id"]):
        raise BrokerError("request_id has invalid syntax")
    if not isinstance(request["project"], str) or not re.fullmatch(r"[a-z0-9][a-z0-9._-]{1,63}", request["project"]):
        raise BrokerError("project has invalid syntax")
    if not isinstance(request["lane"], str) or not 2 <= len(request["lane"]) <= 128:
        raise BrokerError("lane length is invalid")
    ...
    if not profile["quota_domain"].startswith(profile["provider"] + ":sha256:"):
        raise BrokerError("quota domain provider prefix does not match provider")
    if profile.get("requested_effort") is not None and not isinstance(profile.get("requested_effort"), str):
        raise BrokerError("request.profile.requested_effort must be null or a string")
    budget = request.get("budget")
    if not isinstance(budget, dict):
        raise BrokerError("request.budget must be an object")
    limits = {"max_wall_seconds": 21600, "max_turns": 512, "max_context_tokens": 2_000_000}
    for field, maximum in limits.items():
        if isinstance(budget.get(field), bool) or not isinstance(budget.get(field), int) or not 1 <= budget[field] <= maximum:
            raise BrokerError(f"request.budget.{field} must be an integer in [1,{maximum}]")
...
def validate_policy(policy: dict[str, Any]) -> None:
    ...
    if policy["max_concurrent_leases_per_domain"] > 64 or policy["snapshot_max_age_seconds"] > 86400 or policy["lease_max_seconds"] > 21600 or policy["post_reset_quiet_seconds"] > 86400:
        raise BrokerError("policy value exceeds the v1 safety ceiling")
...
def evaluate(
    ...
    for name, estimate in requested.items():
        ...
        if started is not None and parse_time(started, "snapshot.window.window_started_at") > at + dt.timedelta(seconds=5):
            reasons.append("WINDOW_START_FROM_FUTURE")
        reset_at = window.get("resets_at")
        if started is not None and reset_at is not None and parse_time(reset_at, "snapshot.window.resets_at") <= parse_time(started, "snapshot.window.window_started_at"):
            reasons.append("WINDOW_TIMELINE_INVALID")
        if (
...
class Broker:
    ...
    def release(
        ...
        at = at or now_utc()
        if not isinstance(terminal_class, str) or not re.fullmatch(r"[A-Z][A-Z0-9_]{1,63}", terminal_class):
