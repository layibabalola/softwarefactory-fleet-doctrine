...
mkdirSync(tools);
for (const file of ['fleet-sweep.mjs', 'doctrine-sync.mjs', 'Get-FleetHeartbeatStatus.ps1']) {
...
try {
  ...
  assert.deepEqual(sweep.members.map((m) => m.project).sort(), [
    'adobe-ingester', 'cloudvore', 'continuity-autonomous-resumption', 'resumption-parallel-launch-0906',
  ]);
  ...
  assert.deepEqual(sweep.members.map((m) => m.project).sort(), ['adobe-ingester', 'cloudvore']);

  const heartbeat = JSON.parse(runOutput(ps, ['-NoProfile', '-File', join(tools, 'Get-FleetHeartbeatStatus.ps1'), '-BusRoot', bus, '-Json']));
  assert.deepEqual(heartbeat.boards.map((b) => b.board).sort(), ['adobe-ingester', 'cloudvore']);
  assert.equal(heartbeat.boards.find((b) => b.board === 'cloudvore').status, 'ABSENT');
  console.log('fleet membership: old names included before rename; fleet-* names excluded by sweep and heartbeat reader');
} finally {
