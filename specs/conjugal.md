...
## 2026-09-14 delta — fleet factory kernel dogfood (rung: owner-directed Opus 5 session)

```
KERNEL: DOGFOOD-PENDING Sol · fleet-factory-kernel r2 · profile code@r2 · instance NONE in-tree (map in adjudications/factory-kernel/conjugal.md) · since 2026-09-14
```

Pending, not `DOGFOOD`: Conjugal's contract files have exclusive owners (Sol alone writes HUB Status, Decisions and the
Authority Source Index) and doctrine adoption here has gone through Sol ratification. The filing is the steward's own and
is dispositioned by another project's arbiter or the owner, never by Conjugal (kernel §5).
