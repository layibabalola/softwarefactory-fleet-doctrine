...
**Profile of** `specs/fleet-factory-kernel.md` r3. **Profile revision:** r2. **Status:** `BENCHED`. **Benches:** magic-lantern_dannephoto (Canon 5D Mark III firmware), airmypc (release gate on live hardware).
...
| Acceptance evidence (K5) | emulator runs (for example QEMU) are required where inputs can lawfully be obtained and are never sufficient; where inputs cannot lawfully be obtained, the acceptance receipt records why emulator evidence is absent. Acceptance needs a hardware receipt from the declared rig for the exact image digest; the acceptance ledger records both producer and verifier for every accepted subject |
