...
```text
Bootstrap PROMPT A of the fleet doctrine bus (https://github.com/layibabalola/softwarefactory-fleet-doctrine, public, branch master). Do the steps in order and quote each command's output. Never search the disk beyond step 2's list. Never merge, force, reset, clean, stash or switch branches in the doctrine checkout: other sessions share it, and PROMPT A §1 owns its sync. Stop codes: FROZEN, WRONG_PROJECT, UNREADABLE, UNREACHABLE, WRONG_REMOTE. A stop ends the session's work; never work around one. TOOL RULE: run every command with your shell tool and quote only what that call returned in this turn. Never write a command in a code block you did not run; for a command you did not run, write NOT RUN in place of output. Invented output is a failed run.

1. TARGET. T = output of `git rev-parse --show-toplevel` (else this session's working directory). Print T. STOP FROZEN if T's own instructions, or this machine's user-level instructions, mark T read-only, archived or frozen (frozen is per machine: never assume it from a project name). STOP WRONG_PROJECT if T is your home folder, or `git -C "T" remote get-url origin` names softwarefactory-fleet-doctrine. Every write PROMPT A makes goes under T (or, for the readiness receipt of a project whose gates forbid it, the out-of-tree path in PROMPT A §2b), never under the doctrine checkout.
2. FIND D. Try, in order: a path on any line of T's CLAUDE.md or AGENTS.md that contains `softwarefactory-fleet-doctrine` (read those two files whole; nothing else); the folder beside T's main checkout (parent of the first `worktree ` line of `git -C "T" worktree list --porcelain`) + /softwarefactory-fleet-doctrine (if T is not a git repo, the parent of T instead); C:\code\softwarefactory-fleet-doctrine (Windows only); your home directory written out in full (never `~` inside quotes) + /code/softwarefactory-fleet-doctrine. For each: `git -C "<path>" remote get-url origin`. It qualifies if the URL contains layibabalola/softwarefactory-fleet-doctrine, ignoring case; then D = the first `worktree ` line of `git -C "<path>" worktree list --porcelain`. Replace <path> with the real path in every command; never run a command containing < or >.
...
4b. VERIFY. One shell call, D replaced with the real path, all output pasted unedited: `git --version; git --exec-path; git -C "D" config --show-origin --get-regexp "^(url\.|remote\.origin\.)"; git -C "D" ls-remote --get-url https://github.com/layibabalola/softwarefactory-fleet-doctrine; git -C "D" ls-remote https://github.com/layibabalola/softwarefactory-fleet-doctrine refs/heads/master; git -C "D" rev-parse origin/master`. STOP WRONG_REMOTE if any line starts with `url.` (a remote.origin line is normal), if the --get-url line is not exactly that https URL, or if the 40-character hash from ls-remote differs from rev-parse origin/master after one re-run of step 4 and 4b (a push can land between them). If step 4 failed, a hash mismatch is UNREACHABLE instead.
5. READ. `git -C "D" show origin/master:bootstrap/PROMPT-A-sync-and-adopt.md` (the pushed version, not local edits). If that fails, quote `git -C "D" log -1 --format="%H %ci" origin/master` and say which case it is: the step-4 fetch failed (origin/master may be stale), or the fetch succeeded and the file is absent at that commit. Never say the file is absent from the repository. Only when the fetch failed, read D/bootstrap/PROMPT-A-sync-and-adopt.md with your file tool and say it may be stale. Line 1 must start "# PROMPT A", else STOP UNREADABLE. If shell commands are refused anywhere above, STOP UNREADABLE: the human must grant this session the doctrine folder or paste from a project that has it. Do not fetch the file from the web.
6. REPORT, pasting raw output lines exactly as printed (never restructure them): the full ls-remote hash from 4b first (the human checks it against github.com); the 4b block verbatim; T; each step-2 rung and its result; D and how it was found or cloned; the fetch result; the source and line 1 of what you read. Then follow PROMPT A for project T, with D as <doctrine>. If PROMPT A §1 stops, stop, and read nothing further from D. A §1 recovery into `<D>-origin` is not a stop.
...
- **It proves the commands ran.** Measured 2026-09-14 on UltraMagnus: a Haiku session reported a porcelain
  entry with both `branch` and `detached`, a fetch line `[new ref] refs/heads/master`, and an
  `origin/master` of `fa5d65d`. Real git prints none of these (reproduced on git 2.55), and GitHub has
  never held that commit. Step 4b pairs GitHub's live master hash with the local ref, which a narrating
  model cannot guess and a human can check in seconds. The same line also exposes a real
  `url.insteadOf` rewrite or a git shim.
